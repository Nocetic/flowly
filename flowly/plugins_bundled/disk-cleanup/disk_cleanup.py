"""disk_cleanup — ephemeral file cleanup for Flowly.

Library module wrapping deterministic cleanup rules.  The plugin's
``__init__.py`` wires these functions into ``post_tool_call`` and
``on_session_end`` hooks so tracking and cleanup happen automatically
— the agent never needs to call a tool or remember a skill.

Rules:
  - test files    → delete immediately at session end (age >= 0)
  - temp files    → delete after 7 days
  - cron-output   → delete after 14 days
  - empty dirs    → always delete (under FLOWLY_HOME)
...
try:
    ...
    def get_flowly_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("FLOWLY_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".flowly").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$FLOWLY_HOME/logs/``."""
    return get_flowly_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$FLOWLY_HOME/logs/``."""
...
# ── tracked.json — atomic read/write with .bak fallback ────────
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if category not in ALLOWED_CATEGORIES:
        _log(f"WARN: unknown category {category!r}, using 'other'")
        category = "other"
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside FLOWLY_HOME)")
        return False
...
def dry_run() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
_PROTECTED_TOP_LEVEL = {
    ...
    "cache", "skills", "plugins", "disk-cleanup", "credentials",
    "audit", "trajectories", "subagents", "screenshots", "media",
    "workspace", "profiles", "backups", ".worktrees",
}
...
def quick() -> Dict[str, Any]:
    ...
    Returns a summary dict::
        ...
        {
            "deleted":    int,    # files/dirs unlinked
            "empty_dirs": int,    # empty subdirs removed
            "freed":      int,    # bytes reclaimed
            "errors":     [str],  # non-fatal failures
        }
    """
    tracked = load_tracked()
    now = datetime.now(timezone.utc)
    ...
    # Remove empty dirs under FLOWLY_HOME (but leave FLOWLY_HOME itself
    # and the well-known top-level state dirs alone — a fresh install
    # has these empty, and removing them would surprise the user).
    flowly_home = get_flowly_home()
    empty_removed = 0
    try:
        for dirpath in sorted(flowly_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == flowly_home:
                continue
            try:
                rel_parts = dirpath.relative_to(flowly_home).parts
            except ValueError:
                continue
...
# Skip these top-level dirs / config files when guessing categories
# so the cleanup never looks at things the user expects to keep.
_PROTECTED_FOR_GUESS = {
    "disk-cleanup", "logs", "memories", "sessions", "config.json",
    "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
    "credentials", "audit", "workspace", "profiles", "trajectories",
    "subagents", "screenshots", "media",
}


def guess_category(path: Path) -> Optional[str]:
    """Return a category label for *path*, or ``None`` if untrackable.
    ...
    flowly_home = get_flowly_home()
    try:
        rel = path.resolve().relative_to(flowly_home)
        top = rel.parts[0] if rel.parts else ""
        if top in _PROTECTED_FOR_GUESS:
            return None
        ...
        # Path isn't under FLOWLY_HOME (e.g. /tmp/flowly-*) — fall through.
        pass
