---
name: linear
description: "Linear: manage issues, projects, teams, documents via GraphQL + curl (or stdlib Python helper)."
homepage: https://developers.linear.app
metadata: {"flowly":{"emoji":"📌","tags":["linear","project-management","issues","graphql","api","productivity"],"requires":{"bins":["curl","python3"],"env":["LINEAR_API_KEY"]},"related_skills":["github","github-pr-workflow","plan"]}}
---
...
2. Set `LINEAR_API_KEY` in your environment (export it, or add to `~/.flowly/.env`)
...
```bash
SCRIPT=$(find ~/.flowly -path '*skills/linear/scripts/linear_api.py' 2>/dev/null | head -1)
...
The trailing hex segment is the `slugId`. Example: `https://linear.app/acme/document/rfc-permission-gateway-38359beef67c` → `slugId` is `38359beef67c`.
...
- Always use `exec` with `curl` for API calls — do NOT use `web_fetch` or browser tools
