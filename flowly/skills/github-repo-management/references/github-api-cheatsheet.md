...
If `gh` is authenticated, prefer `gh api`. For raw `curl`, set `$GITHUB_TOKEN`, `$GH_OWNER`, and `$GH_REPO` from the current repo or environment:
```bash
GITHUB_TOKEN="${GITHUB_TOKEN:?set GITHUB_TOKEN or use gh api instead}"
```
