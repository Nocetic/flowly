...
metadata: {"flowly":{"emoji":"🐍","tags":["debugging","python","pdb","debugpy","breakpoints","dap","post-mortem"],"requires":{"bins":["python3"]},"install":[{"id":"pip","kind":"pip","package":"debugpy remote-pdb","label":"Install debugpy and remote-pdb (pip)"}],"related_skills":["systematic-debugging","node-inspect-debugger"]}}
---
...
| **`debugpy`** | Remote / headless / "attach to already-running process." Talks DAP, scriptable from exec, works for long-lived processes (gateway, daemon, PTY children). |
...
- A long-running process (Flowly gateway, daemon, worker) misbehaves and you can't restart it
...
The flowly test runner and pytest both support this:
...
For long-lived processes: Flowly gateway, a daemon, a worker, or a process that's already misbehaving and can't be restarted clean.
...
### Connecting a client from the exec

The easiest exec-side DAP client is VS Code CLI or a small script. You have two practical options:
...
**Option 3: Ditch DAP, use `remote-pdb`** — usually what you actually want from a exec agent:
...
## Debugging Flowly / Python Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### CLI or one-shot scripts
Easiest: add `breakpoint()` near the suspect line, then run the command normally. Control returns to your exec session at the pause point.

### Gateway / worker subprocesses
Options:

**A. Source-edit the gateway:**
```python
# near the top of the target entrypoint or handler
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```
Start the process. It will wait until a debugger attaches; execution resumes when you `continue`.
...
```
Trigger the matching code path, then `nc 127.0.0.1 4444` in another exec session.

### Persistent subprocesses
Same pattern: use `remote-pdb` with `set_trace()` inside the target path. The first trigger blocks until you connect; subsequent calls pass through normally unless you re-arm.

### Long-lived gateway
Use `remote-pdb` at a handler, or `debugpy` with `--wait-for-client` if you're restarting the gateway anyway.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Flowly subagents, debug one process at a time.
