...
- `generate_video()` split: the Pollinations text-to-image call was removed.
  Pair Flowly image generation with `pixel_art()` for base frames when image
  generation is available. Only the overlay + ffmpeg encoding remains.
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — use Flowly image generation when available
...
- License: MIT (inherits from flowly repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Flowly contributors
