...
description: "Extract text from PDFs and scanned documents (pymupdf for fast text, marker-pdf for OCR/equations/forms)."
metadata: {"flowly":{"emoji":"📄","tags":["pdf","documents","research","arxiv","text-extraction","ocr"],"requires":{"bins":["python3"]},"install":[{"id":"pymupdf","kind":"pip","package":"pymupdf pymupdf4llm","label":"Install pymupdf (pip, ~25MB)"}],"related_skills":["arxiv","summarize","nano-pdf"]}}
---
...
python3 scripts/extract_marker.py document.pdf --output_dir out/  # Save images
python3 scripts/extract_marker.py scanned.pdf                 # Scanned PDF (OCR)
...
See the `arxiv` skill for direct API queries and the Semantic Scholar workflow.

## Split, Merge & Search

pymupdf handles these natively — use inline Python via `exec`:
