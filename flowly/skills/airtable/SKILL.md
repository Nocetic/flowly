---
name: airtable
description: "Airtable REST API via curl. Records CRUD, filters, upserts. Requires AIRTABLE_API_KEY (PAT)."
homepage: https://airtable.com/developers/web/api/introduction
metadata: {"flowly":{"emoji":"🗃️","tags":["airtable","productivity","database","api"],"requires":{"bins":["curl","python3"],"env":["AIRTABLE_API_KEY"]},"related_skills":["notion","linear","summarize"]}}
---
...
4. Store the token in `~/.flowly/.env` (or via `flowly setup`):
   ```
...
- **Always use the `exec` tool with `curl`.** Do NOT use `web_fetch` (it can't send auth headers) or browser-style tools (needs UI auth and is slow).
- **`AIRTABLE_API_KEY` flows from `~/.flowly/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
