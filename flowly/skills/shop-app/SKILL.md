---
name: shop-app
description: "Shop.app personal shopping assistant: product search across stores, price compare, similar items, order tracking, returns, reorder. Auth via device-authorization for per-user ops."
metadata: {"flowly":{"emoji":"🛒","tags":["shop-app","shopping","e-commerce","products","orders","returns"],"requires":{"bins":["curl"]}}}
---
...
Flowly's gateway adapters (Telegram, Discord, Slack, iMessage, …) render markdown and image URLs automatically. Write normal markdown with image URLs on their own line — the adapter handles platform-specific layout. Do **not** invent a `message()` tool call (that belongs to Shop.app's own runtime, not Flowly).
