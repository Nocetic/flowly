...
../assets/, and writes a setup.sh that creates Flowly profiles and fires the
initial kanban task.

The generated setup.sh follows Flowly's single-shared-workspace model: every
profile gets a SOUL.md, profile configs are patched in place, the task graph
is written to TEAM.md, and the initial kanban task is created against one
`--workspace dir:<path>` so the whole team collaborates in the same directory.
    ...
    if "team" in plan:
        if not isinstance(plan["team"], list) or not plan["team"]:
            ...
            for i, t in enumerate(plan["team"]):
                ...
                # Profile name must match Flowly's regex (lowercase
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Flowly profile rules"
                        )
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for key in plan.get("api_keys_required", []):
        key_checks.append(f'check_key {key} flowly {key} || exit 1')
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'flowly profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.flowly/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
