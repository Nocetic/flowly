...
flowly kanban watch --tenant <project-slug>

# Snapshot of the board
flowly kanban list --tenant <project-slug>
flowly kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
flowly kanban stats --tenant <project-slug>

# Visual dashboard (browser)
flowly dashboard

# Inspect a specific task (includes comments + events)
flowly kanban show <task-id>

# Follow a single task's event stream
flowly kanban tail <task-id>
```

Verify available subcommands with `flowly kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `flowly kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `flowly kanban block <id>` then `flowly kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `flowly kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `flowly kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `flowly profile list` (profile exists?), `flowly status` (gateway/dispatcher up?), and verify tenant. |
...
flowly kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
flowly kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(flowly kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
flowly kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
flowly kanban block <task-id>
flowly kanban archive <task-id>

# Diagnose what happened
flowly kanban show <task-id>      # task body, comments, recent events
flowly kanban tail <task-id>      # follow the live event stream
flowly kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `flowly kanban create` for the director
...
while true; do
    clear
    flowly kanban list --tenant <slug>
    echo "---"
    flowly kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `flowly kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `flowly profile list` and start
