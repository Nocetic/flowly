...
the actual `setup.sh` that creates the project workspace, configures Flowly
...
> **Conventions:** this skill standardizes four setup rules so a video team
> behaves predictably across projects and styles — (1) one shared project
> workspace per video, (2) a per-profile `SOUL.md` that pins each role's job,
> (3) profile configs patched in place from the plan, and (4) every kanban
> call bound to the same `--workspace dir:<path>` so the team collaborates in
> one directory.
...
2. **Create profiles** — `flowly profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.flowly/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `flowly kanban create` assigned to the director
...
```bash
flowly profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.flowly/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.flowly/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Flowly
...
Each profile gets a `SOUL.md` at `~/.flowly/profiles/<name>/SOUL.md` that
...
```bash
flowly kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.flowly/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.flowly/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.flowly/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.flowly/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY flowly ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY flowly OPENROUTER_API_KEY || exit 1
# ...
```
