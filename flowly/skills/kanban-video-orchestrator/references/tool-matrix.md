...
Maps each role archetype to the Flowly skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public flowly
...
## Flowly skills relevant to video production

### Visual / rendering skills (`flowly/skills/creative/`)
...
### Design / pre-production skills (`flowly/skills/creative/`)
...
### Audio / media skills (`flowly/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`flowly/skills/devops/`)
...
These are **not** Flowly skills but external CLIs / APIs that profiles invoke.
...
## Built-in Flowly tools for media review

These are native Flowly tools — not invoked via terminal but through their own
...
key is present in `~/.flowly/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Flowly profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.flowly/.env` → environment variable.
...
When a new Flowly-public video skill ships:
