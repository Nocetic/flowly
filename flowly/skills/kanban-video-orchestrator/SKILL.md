...
description: "Plan and run a multi-agent video production pipeline. Decompose any video brief — narrative film, product/marketing clip, music video, explainer, ASCII/terminal art, generative loop, comic, 3D, real-time — into specialised profiles (writer, designer, animator, renderer, voice, editor) coordinated through a kanban board. Generates setup scripts, monitors execution, intervenes on stalls. References in references/, helper scripts in scripts/."
metadata: {"flowly":{"emoji":"🎬","tags":["video","kanban","multi-agent","orchestration","production-pipeline"]}}
---

# Kanban Video Orchestrator

Wrap any video request — from a 15-second product teaser to a 5-minute narrative
short to a music video to an ASCII loop — in a Flowly Kanban pipeline that
...
3. **Generates** a setup script that creates Flowly profiles, project workspace, and the initial kanban task
...
For mapping role → which Flowly skills + toolsets it loads, see
...
3. Creates each Flowly profile via `flowly profile create --clone`
...
7. Fires the initial `flowly kanban create` task assigned to the director
...
```bash
flowly kanban watch --tenant <project-tenant>     # live events
flowly kanban list  --tenant <project-tenant>     # board snapshot
flowly dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `flowly kanban create` call to a specialist profile. The
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `~/.flowly/.env` or the user's secret store.
