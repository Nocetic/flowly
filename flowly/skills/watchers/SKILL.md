...
metadata: {"flowly":{"emoji":"⏱️","tags":["cron","polling","rss","github","http","automation","monitoring"],"requires":{"bins":["python3","curl"]},"category":"devops","related_skills":["blogwatcher","github"]}}
---

# Watchers

Poll external sources on an interval and react only to new items. Three ready-made scripts plus a shared watermark helper; wire them into a cron job (or run them ad-hoc with `exec`).
...
The scripts below handle all three. The agent runs them via `exec` — from a cron job, a webhook, or an interactive chat — and reports what's new.

## Ready-made scripts

All three live in `${FLOWLY_HOME:-$HOME/.flowly}/skills/watchers/scripts/` once the skill is installed. Each reads `WATCHER_STATE_DIR` (defaults to `${FLOWLY_HOME:-$HOME/.flowly}/watcher-state/`) for its state file, keyed by the `--name` argument.
...
Run a watcher directly with `exec`:

```bash
python ${FLOWLY_HOME:-$HOME/.flowly}/skills/watchers/scripts/watch_rss.py \
  --name hn --url https://news.ycombinator.com/rss --max 5
```

Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.flowly/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python ${FLOWLY_HOME:-$HOME/.flowly}/skills/watchers/scripts/watch_github.py \
  --name repo-issues --repo octocat/hello-world --scope issues
```

Poll an arbitrary JSON API:

```bash
python ${FLOWLY_HOME:-$HOME/.flowly}/skills/watchers/scripts/watch_http_json.py \
...
The agent invokes the script via `exec` inside the cron job's agent loop; no changes to cron's built-in `--script` flag are needed.

## State files

Every watcher writes `${FLOWLY_HOME:-$HOME/.flowly}/watcher-state/<name>.json`. Inspect:

```bash
cat ${FLOWLY_HOME:-$HOME/.flowly}/watcher-state/hn.json
```

Force a replay (next run treated as first poll):

```bash
rm ${FLOWLY_HOME:-$HOME/.flowly}/watcher-state/hn.json
```
...
4. **Putting the state dir where the agent's sandbox can't write.** `${FLOWLY_HOME:-$HOME/.flowly}/watcher-state/` is the default writable state location. Docker/Modal backends may not see arbitrary host paths.
