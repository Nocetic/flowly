...
Usage (via cron with --no-agent):
    ...
    flowly cron create repo-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "${FLOWLY_HOME:-$HOME/.flowly}/skills/watchers/scripts/watch_github.py" \\
      --script-args "--name repo-issues --repo octocat/hello-world --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.flowly/.env to avoid the 60 req/hr
