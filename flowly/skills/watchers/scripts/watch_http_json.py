...
Usage (via cron with --no-agent):
    ...
    flowly cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "${FLOWLY_HOME:-$HOME/.flowly}/skills/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Flowly-Watcher/1.0"})
