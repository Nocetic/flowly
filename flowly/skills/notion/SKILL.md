---
name: notion
description: "Notion API + ntn CLI: pages, databases (data sources), markdown, Workers. Requires NOTION_API_KEY."
homepage: https://developers.notion.com
metadata: {"flowly":{"emoji":"📓","tags":["notion","productivity","notes","database","api","cli","workers"],"requires":{"bins":["curl"],"env":["NOTION_API_KEY"]},"install":[{"id":"ntn","kind":"shell","cmd":"curl -fsSL https://ntn.dev | bash","bins":["ntn"],"label":"Install ntn CLI (optional, macOS/Linux)"}],"related_skills":["llm-wiki","summarize"]}}
---
...
Add those exports to your shell profile (or to `~/.flowly/.env`) so every session inherits them.
...
curl -s -X PATCH "https://api.notion.com/v1/blocks/{page_id}/children" \
  ...
  -d '{
    "children": [
      {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": "Hello from Flowly!"}}]}}
    ]
  }'
```
...
See `references/block-types.md` for the full block-type catalog with examples.
