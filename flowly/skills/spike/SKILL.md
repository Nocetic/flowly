...
metadata: {"flowly":{"emoji":"🧪","tags":["spike","prototype","experiment","feasibility","throwaway","exploration","research","planning","mvp","proof-of-concept"],"related_skills":["writing-plans","subagent-driven-development","plan"]}}
---
...
If a richer project-specific spike workflow is installed as a sibling skill, prefer it when the user wants persistent spike state, MANIFEST tracking across sessions, Given/When/Then verdict format, and commit patterns that integrate with a broader spec-driven development pipeline. This skill is the lightweight standalone version.
...
Use Flowly tools for the research step:

- `web_search("python websocket streaming libraries 2025")` — find candidates
- `web_fetch(url="https://websockets.readthedocs.io/...")` — read the actual docs when a URL is known
...
**Parallel comparison spikes (002a / 002b) — delegate.** When configured subagents are available and two approaches can run in parallel, use `delegate_to` once per subagent with a precise message:

```
delegate_to(agent_id="coder-a", message="Build spike 002a-pdf-parse-pdfjs in spikes/002a-pdf-parse-pdfjs. Return README verdict plus changed files.")
delegate_to(agent_id="coder-b", message="Build spike 002b-pdf-parse-camelot in spikes/002b-pdf-parse-camelot. Return README verdict plus changed files.")
```
