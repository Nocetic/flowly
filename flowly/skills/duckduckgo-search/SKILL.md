...
metadata: {"flowly":{"emoji":"🦆","tags":["search","duckduckgo","web-search","free","fallback"],"requires":{"bins":["ddgs"]},"install":[{"id":"pip","kind":"pip","package":"ddgs","bins":["ddgs"],"label":"Install ddgs (pip)"}],"related_skills":["arxiv","summarize"]}}
---
...
Decision tree:
1. If `ddgs` CLI is installed, prefer `exec` + `ddgs`
2. If `ddgs` CLI is missing, do not assume the Python runtime can import `ddgs`
...
- Shell commands and Python snippets may use different environments
- A successful shell install does not guarantee a different Python interpreter can import `ddgs`
- Never assume third-party Python packages are preinstalled
...
Use the `ddgs` command via `exec` when it exists. This is the preferred path because it avoids assuming a Python package is importable.
...
Use the `DDGS` class in Python only after verifying that `ddgs` is installed for that interpreter. Do not assume Python includes third-party packages by default.

Safe wording:
- "Use Python with `ddgs` after installing or verifying the package if needed"

Avoid saying:
- "Python includes `ddgs`"
- "DuckDuckGo search works by default in Python"
...
- **Separate runtimes**: A successful `ddgs` install in one environment does not automatically mean every Python interpreter can import it.
...
| CLI works but Python import fails | Shell and Python are using different environments | Keep using CLI, or separately prepare the Python runtime |
...
- **Do not assume Python can import `ddgs`**: `from ddgs import DDGS` may fail with `ModuleNotFoundError` unless that runtime was prepared separately.
