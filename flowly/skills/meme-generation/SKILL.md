...
metadata: {"flowly":{"emoji":"🖼️","tags":["creative","memes","humor","images"],"requires":{"bins":["python3"]},"category":"creative","related_skills":["ascii-art","pixel-art"]}}
---
...
4. Find the skill's script directory:
   ```
   SKILL_DIR=$(dirname "$(find ~/.flowly/skills -path '*/meme-generation/SKILL.md' 2>/dev/null | head -1)")
   ```
5. Run the generator:
   ```bash
...
2. If an image generation capability is available, create a scene that matches the meme concept. Do NOT include any text in the image prompt — text will be added by the script. Describe only the visual scene.
3. Find the generated image path from the result URL. Download it to a local path if needed.
...
5. **Verify visually** if an image inspection capability is available: check that the text is legible, well-positioned, and the meme reads clearly. If the review flags issues, try the other mode (switch between overlay and bars) or regenerate the scene.
6. Return the image with `MEDIA:/tmp/meme.png`
