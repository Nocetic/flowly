---
name: here.now
description: "Publish static sites to {slug}.here.now and store private files in cloud Drives for agent-to-agent handoff. Helper scripts in scripts/ (publish.sh, drive.sh)."
metadata: {"flowly":{"emoji":"🌐","tags":["here.now","publish","deploy","hosting","static-site","drive","storage"],"requires":{"bins":["curl","file","jq"]}}}
---
...
- Skill helper paths:
  - `${FLOWLY_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${FLOWLY_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${FLOWLY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client flowly
```
...
```bash
PUBLISH="${FLOWLY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client flowly
```
...
```bash
DRIVE="${FLOWLY_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `flowly`)    |
