...
class MessageTool(Tool):
    ...
    def description(self) -> str:
        return (
            "Send a message to the user on a specific chat channel (Telegram, "
            "WhatsApp, etc.), optionally with media attachments (images, "
            "documents). Use media_paths to attach files like screenshots. "
            "The content will be sent as the caption for media messages.\n\n"
            "IMPORTANT — only call this tool when you need to route content "
            "to a named channel. For normal conversation in the current "
            "thread, reply with text directly. Do NOT wrap a conversational "
            "reply in this tool."
        )
    ...
    async def execute(
        ...
        **kwargs: Any
        ...
        # Defensive coercion for malformed tool arguments (see
        # `_arg_coercion.coerce_str_list` for rationale). Zero-op for the
        # macOS-typical case where media_paths is already a real list — this
        # code path only does work when a smaller model emits the argument
        # as a stringified JSON array (observed on Haiku + Windows paths).
        media_paths = coerce_str_list(
            media_paths, param_name="media_paths", tool_name="message"
        )
