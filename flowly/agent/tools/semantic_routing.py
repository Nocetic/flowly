...
class FastEmbedLocalEncoder:
    ...
    def _resolve_model(cache_dir: Path, *, allow_download: bool) -> Path:
        ...
        if bundled:
            ...
            # Import the download client only for the cache/download path.
            # Packaged desktop distributions must be able to verify and use
            # their bundled model without this optional dependency installed.
...
class SemanticToolRouter:
    ...
    def _dot(left: Sequence[float], right: Sequence[float]) -> float:
        if len(left) != len(right):
            # A dimension mismatch means the derived index and encoder are no
            # longer compatible. Treating it as a low semantic score hides a
            # corrupt/stale index and disables the intended lexical fallback.
