...
_CHANNELS: list[IntegrationCard] = [
    ...
    IntegrationCard(
        ...
        fields=[
            ...
            Field("group_policy", "Group policy", FieldType.SELECT,
                  default="mention",
                  choices=[("mention", "mention · only when @bot is tagged or replied to"),
    ...
    IntegrationCard(
        key="buzz", label="Buzz", category="channel",
        description="Join a Buzz community through its Nostr relay.",
        docs_url="https://github.com/block/buzz",
        config_path="channels.buzz",
        fields=[
            _enabled_field(),
            Field(
                "relay_url",
                "Community relay URL",
                FieldType.TEXT,
                placeholder="https://community.communities.buzz.xyz",
                required=True,
                help="The HTTP(S) base URL for the Buzz community relay.",
            ),
            Field(
                "private_key",
                "Nostr private key",
                ...
                placeholder="nsec1…",
                required=True,
                help="Stored as BUZZ_PRIVATE_KEY in this profile's private .env file.",
                env_var="BUZZ_PRIVATE_KEY",
            ),
            Field(
                "channels",
                "Watched channels",
                FieldType.MULTI,
                placeholder="channel UUID, another UUID",
                help="Optional. Empty automatically watches every joined channel.",
            ),
            Field(
                "home_channel",
                "Home channel",
                ...
                help="Optional. Flowly automatically uses the first watched or joined channel.",
            ),
            Field(
                "group_policy",
                "Channel policy",
                ...
                default="mention",
                choices=[
                    ("mention", "mention · respond only when addressed"),
                    ("open", "open · respond to every message"),
                ],
            ),
            Field(
                ...
                FieldType.BOOL,
                default=False,
                help="Off by default. Otherwise only Allowed senders can interact.",
            ),
            Field(
                "allow_from",
                "Allowed senders",
                FieldType.MULTI,
                placeholder="npub1…, 64-character public key",
                help="Nostr npubs or hex public keys. Empty denies everyone unless open access is on.",
            ),
            Field(
                "transport",
                "Inbound transport",
                ...
                default="auto",
                choices=[
                    ("auto", "auto · WebSocket with polling fallback"),
                    ("websocket", "websocket · require live Nostr transport"),
                    ("poll", "poll · Buzz CLI polling only"),
                ],
            ),
            Field(
                ...
                FieldType.INT,
                default=4,
                help="Polling fallback frequency; values below one second are clamped.",
            ),
            Field(
                "cli_path",
                "Buzz CLI path",
                FieldType.TEXT,
                placeholder="/usr/local/bin/buzz",
                help="Optional. Defaults to buzz on PATH, then ~/bin/buzz.",
            ),
            Field(
                "credentials_file",
                "Credentials file",
                FieldType.TEXT,
                placeholder="~/.config/buzz/credentials.json",
                help="Optional fallback JSON containing nsec or private_key_hex.",
            ),
            Field(
                "auth_tag",
                "NIP-OA auth tag",
                FieldType.PASSWORD,
                placeholder='["auth", "…", "", "…"]',
                help="Optional owner-attestation tag used during NIP-42 authentication.",
            ),
        ],
        probe=probe_buzz,
        discover=discover_buzz,
    ),
    IntegrationCard(
